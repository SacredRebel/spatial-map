// Look at the world: node scripts/shot.mjs "<url>" out.png [lng,lat,heading]
//
//   Boots the built world in headless Chromium against whatever the url names, waits for the
//   ground and the aerial, optionally stands the player somewhere, and writes a screenshot.
import { chromium } from 'playwright';
import { existsSync } from 'fs';

const [url, out, at] = process.argv.slice(2);
...
console.log('wrote', out, await page.evaluate(() => JSON.stringify({ ...window.world.state(), veg: window.world.vegetation.counts, today: window.world.today.counts, drape: window.world.terrain.imageryState })));
await browser.close();
